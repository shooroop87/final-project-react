...
export const UserCardUI: FC<UserCardUIProps> = ({ card, type, setLike, isLiked }) => {
  ...
  // Метод по созданию списка навыков
  const renderSkills = (skills: TSkill[], limit = 2) => {
    ...
    return (
      <>
        ...
        {restCount > 0 && (
          <div className={clsx(styles.skill_item, styles.other, styles.plus)}>
            {`+${restCount}`}
          </div>
        )}
      </>
  ...
  return (
    <article className={clsx(styles.card, type === 'full' && styles.card_full)}>
      <div className={styles.info}>
        ...
        <div className={styles.user_data}>
          ...
          <span className={styles.data_other}>
            {`${card.city}, ${formatAge(card.age)}`}
          </span>
        </div>
      ...
      <div className={clsx(styles.skills_section, type === 'full' && styles.skills_section_full)}>
        <div
          className={clsx(
            styles.skills_container,
            type === 'full' && styles.skills_container_full
          )}
        >
